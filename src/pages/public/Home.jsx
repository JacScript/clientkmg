      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
          <div className="text-center p-8 max-w-md">
            ...
            <button onClick={() => window.location.reload()} className="px-6 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors">
              Refresh Page
            </button>
          </div>
        </div>
  ...
  <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
    <div className="text-center max-w-md mx-auto">
      ...
      <div className="space-y-3">
        <button onClick={onRetry} className="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
          Try Again
        </button>
        <button onClick={() => (window.location.href = "/")} className="w-full sm:w-auto block sm:inline-block ml-0 sm:ml-3 px-6 py-3 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2">
          Go Home
        </button>
      </div>
    </div>
...
const AnimatedSection = ({ id, children, className = "", delay = 0 }) => {
  ...
  return (
    <section id={id} className={`transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"} ${className}`}>
      {children}
    </section>
  );
};
...
const SafeComponent = ({ children, fallback = null, name = "Component" }) => {
  try { return children(); }
    ...
    if (process.env.NODE_ENV === "development") console.error(`${name} crashed:`, error);
...
const HomeContent = ({ homeData, testimonialData }) => {
  const testimonials = useMemo(() => extractTestimonials(testimonialData), [testimonialData]);
  return (
    <div className="max-w-screen overflow-x-hidden">
      ...
      <SafeComponent name="ContactUs" fallback={<div className="h-20 bg-gray-50" />}>
...
const Home = () => {
  ...
  const { data: resData, isLoading: homeLoading, isError: homeError, error: homeErrorObj, refetch: refetchHome, isFetching: homeFetching } = useQuery({
    queryKey: ["homepage"],
  ...
  const { data: testimonialData, isError: testimonialError, refetch: refetchTestimonials } = useQuery({
    ...
    onError: (err) => { console.error("[Home] Testimonials fetch failed:", err?.response?.status, err?.message); },
  });

  const homeData = useMemo(() => { try { return resData?.data?.data; } catch { return null; } }, [resData]);
  const handleRetry = () => { refetchHome(); refetchTestimonials(); };
  ...
  useEffect(() => {
    if (homeData) {
      const structuredData = { "@context": "https://schema.org", "@type": "WebPage", "name": "Home", "description": "Experience extraordinary travel and services", "url": window.location.href };
  ...
  if (homeLoading && !homeData) return <LoadingSpinner />;
  if (homeError) return <ErrorState onRetry={handleRetry} error={homeErrorObj} />;
