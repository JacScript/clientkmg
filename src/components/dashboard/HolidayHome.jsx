...
import { keepPreviousData, useQueryClient, useMutation, useQuery } from '@tanstack/react-query';
import {
  getHolidayHomePageData,
  updateHolidayHomeSection,
  createApartment,
  updateApartment,
  deleteApartment,
} from '../../http';
...
import LoadingSpinner from '../LoadingComponents';
import ErrorDisplay from '../ErrorComponent';
import HeroSectionForm from './holidayHomeComponent/HeroSectionForm';
import ListingsCopyForm from './holidayHomeComponent/ListingsCopyForm';
import ApartmentsGrid from './holidayHomeComponent/ApartmentsGrid';
import AddApartmentModal from './holidayHomeComponent/Addapartmentmodal';
...
const HolidayHome = ({ activeSection }) => {
  const [visibleSection, setVisibleSection] = useState('heroSection');
  const [heroForm, setHeroForm] = useState({ eyebrow: '', title: '', subheading: '', images: [] });
  const [listingsForm, setListingsForm] = useState({ badge: '', heading: '', subheading: '' });
  const [editingApartmentId, setEditingApartmentId] = useState(null);
  const [draftApartment, setDraftApartment] = useState(null);
  ...
  // listingsSection.apartments arrives already populated with full
  // Apartment documents — the GET route does .populate() server-side.
  const apartments = pageData?.listingsSection?.apartments || [];
  ...
  const sectionMutation = useMutation({
    mutationFn: ({ sectionName, data }) => updateHolidayHomeSection(pageData?._id, sectionName, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['holiday-home'] });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      // Backend auto-links new apartments to this page on creation, so a
      // refetch is all that's needed to see it appear below.
      queryClient.invalidateQueries({ queryKey: ['holiday-home'] });
      enqueueSnackbar('Apartment created!', { variant: 'success' });
      setIsAddModalOpen(false);
    },
    onError: (error) => {
      enqueueSnackbar('Failed to create apartment: ' + error.message, { variant: 'error' });
    },
  });
  ...
  const updateApartmentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['holiday-home'] });
      enqueueSnackbar('Apartment updated!', { variant: 'success' });
      setEditingApartmentId(null);
      setDraftApartment(null);
    },
    onError: (error) => {
      enqueueSnackbar('Failed to update apartment: ' + error.message, { variant: 'error' });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      // Backend also pulls the deleted apartment out of this page's
      // listingsSection.apartments — a refetch reflects that automatically.
      queryClient.invalidateQueries({ queryKey: ['holiday-home'] });
      enqueueSnackbar('Apartment deleted', { variant: 'info' });
    },
    onError: (error) => {
  ...
  const handleSaveHero = () => {
    ...
    sectionMutation.mutate({ sectionName: 'heroSection', data: heroForm });
  };

  // The section PATCH route replaces the whole section object, so the
  // apartments array must be included here too — otherwise saving the
  // listings copy would wipe out every linked apartment.
  const handleSaveListingsCopy = () => {
    if (!pageData?._id) return;
    sectionMutation.mutate({
      sectionName: 'listingsSection',
      data: { ...listingsForm, apartments: apartments.map((a) => a._id) },
    });
  };
  ...
  const handleDraftChange = (field, value) => {
    setDraftApartment((prev) => ({ ...prev, [field]: value }));
  };

  const handleSaveApartment = () => {
    if (!draftApartment?._id) return;
    updateApartmentMutation.mutate({ id: draftApartment._id, data: draftApartment });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <header className="bg-white shadow sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <h1 className="text-3xl font-bold text-gray-900">Holiday Home Management</h1>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {visibleSection === 'listingsSection' && (
          <div className="space-y-8">
            <ListingsCopyForm
              listingsForm={listingsForm}
              setListingsForm={setListingsForm}
              onSave={handleSaveListingsCopy}
              isSaving={sectionMutation.isPending}
            />
            ...
            <ApartmentsGrid
              ...
              draftApartment={draftApartment}
              onAddClick={() => setIsAddModalOpen(true)}
              onEdit={startEditApartment}
              onCancel={cancelEditApartment}
              onSave={handleSaveApartment}
              onDelete={handleDeleteApartment}
              onChange={handleDraftChange}
              onImagesChange={(images) => handleDraftChange('images', images)}
              isSaving={updateApartmentMutation.isPending}
            />
          </div>
        )}
      </main>
