...
const WhySection = ({ whySection, handleInputChange }) => {
  ...
  const uploadToCloudinary = useCallback(async (file) => {
    try {
      ...
      handleInputChange('whySection', 'image', 'uploading...');
      ...
      handleInputChange('whySection', 'image', data.secure_url);
      
    } catch (error) {
      console.error('Cloudinary upload error:', error);
      alert('Failed to upload image. Please try again.');
      // Clear the uploading state
      handleInputChange('whySection', 'image', '');
    }
  }, [handleInputChange]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2">
          <label className="block text-sm font-medium text-gray-700 mb-3">Why Choose Us Section Image</label>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              <div 
                className={`border-2 border-dashed rounded-md p-6 flex flex-col items-center justify-center h-48 transition-colors cursor-pointer ${
                  isDraggedOver 
                    ...
                    : whySection?.image && whySection.image !== 'uploading...'
              ...
              >
                {whySection?.image === 'uploading...' ? (
                  ...
                  <>
                    <img
                      src={getOptimizedImageUrl(whySection.image)}
                      alt="Why choose us section preview"
            ...
            <div className="flex flex-col">
              ...
              <input
                type="text"
                value={whySection?.image === 'uploading...' ? '' : whySection?.image || ''}
                onChange={(e) => handleInputChange('whySection', 'image', e.target.value)}
                placeholder="https://example.com/image.jpg"
                disabled={whySection?.image === 'uploading...'}
              ...
              {whySection?.image && 
               whySection.image !== 'uploading...' && 
               whySection.image.startsWith('http') && (
                <div className="mt-3">
              ...
              {whySection?.image && whySection.image !== 'uploading...' && (
                <button
                  type="button"
                  onClick={() => handleInputChange('whySection', 'image', '')}
